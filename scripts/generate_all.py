...
def main():
    sigmaDir = sys.argv[ 1 ]
    sigmacDir = sys.argv[ 2 ]
    rootOutputDir = sys.argv[ 3 ]
    dirsToProcess = sys.argv[ 4 : ]
        ...
        for d in dirsToProcess:
            ...
            for r, _, files in os.walk( '%srules/%s/' % ( sigmaDir, d, ) ):
                for f in files:
                    ...
                    pool.apply_async( processRule, ( sigmaDir, sigmacDir, thisFile, outFile ) )
        pool.close()
        pool.join()

def processRule( sigmaDir, sigmacDir, thisFile, outFile ):
    ...
    # Artifact rules.
    os.system( "python3 %stools/sigmac -t limacharlie --backend-option lc_target=artifact -c %stools/config/limacharlie.yml %s > %s" % ( sigmacDir, sigmacDir, thisFile, outFile ) )
    ...
    try:
        ...
        pass
    os.system( "python3 %stools/sigmac -t limacharlie --backend-option lc_target=edr -c %stools/config/limacharlie.yml %s > %s" % ( sigmacDir, sigmacDir, thisFile, outFile ) )
    if os.path.getsize( outFile ) == 0:
        print( "rule %s was empty, deleting" % ( outFile, ) )
        os.system( 'rm %s' % ( outFile, ) )
